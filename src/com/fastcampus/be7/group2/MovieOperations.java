    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (menu.equalsIgnoreCase("I")) {
                ...
                if (movieProcessor.getMaxMovieCount() == 0) {
                    // - 먼저, 영화 데이터의 수를 입력하고
                    System.out.print("영화 데이터의 수를 입력해주세요 : ");
                    movieProcessor.initMovie(scanner.nextInt());
                    scanner.nextLine();
                }
                ...
                if (movieProcessor.getMaxMovieCount() == movieProcessor.getCurrentMovieCount()) {
                    movieProcessor.increaseMaxCount();
                }
                // MovieDTO 저장
                MovieDTO movieDTO = new MovieDTO(title, major, runningTime, rating, genre);
                movieProcessor.addMovie(movieDTO);
                System.out.println("영화 저장이 완료되었습니다.");
                System.out.println("총 " + movieProcessor.getCurrentMovieCount() + "개의 영화가 저장되어 있습니다.");
                System.out.println();
                ...
                if (movieProcessor.getCurrentMovieCount() > 0) {
                    System.out.println("총 " + movieProcessor.getCurrentMovieCount() + "개의 영화가 있습니다.");
                    MovieDTO[] movies = movieProcessor.getMovieList();
                ...
                while (true) {
                    if (movieProcessor.getCurrentMovieCount() > 0) {
                        ...
                        int genre = scanner.nextInt();
                        if (genre > 0 && genre < 4) {
                            int searchCount = movieProcessor.searchCount(genre);
                            if (searchCount == 0) {
                                System.out.println("선택하신 장르의 영화가 없습니다!");
                            } else {
                                System.out.println("요청하신 장르의 영화가 " + movieProcessor.searchCount(genre) + "건 있습니다.");
                                MovieDTO[] movies = movieProcessor.searchMovieByGenre(genre);
                                for (MovieDTO movie : movies) {
                                    System.out.println(movie.printMovieInfo());
                                }
                            }
